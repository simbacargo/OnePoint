...
const SalesListScreen = () => {
  ...
  const salesStats = useMemo(() => {
    ...
    return {
      totalSales: totalSales.toLocaleString(undefined, {
        minimumFractionDigits: 2,
      }),
      transactionCount,
      averageSale: averageSale.toLocaleString(undefined, {
        minimumFractionDigits: 2,
      }),
    };
  }, [sales]);
  ...
  const fetchSales = useCallback(async (showLoading = true) => {
    ...
    const token = await AsyncStorage.getItem("@authToken"); // Ensure token is stored in AsyncStorage after login
    try {
      ...
      Alert.alert(
        "Error",
        "Could not fetch sales list. Check network connection.",
      );
    } finally {
      setIsLoading(false);
    ...
    <View style={listStyles.saleItem}>
      <View style={listStyles.headerRow}>
        ...
        <Text style={listStyles.saleDate}>
          ...
          {new Date(item.date_sold).toLocaleTimeString([], {
            hour: "2-digit",
            minute: "2-digit",
          })}
        </Text>
      ...
      <Text style={listStyles.productName} numberOfLines={1}>
        {item.product_name}
      </Text>
      ...
      <View style={listStyles.detailRow}>
        ...
        <Text style={listStyles.detailText}>
          Price: {parseFloat(item.price_per_unit).toFixed(0)}
        </Text>
      </View>

      <View style={listStyles.totalRow}>
        <Text style={listStyles.totalLabel}>TOTAL:</Text>
        <Text style={listStyles.totalValue}>
          {thousandSeparator(parseFloat(item.total_amount).toFixed(0))}
        </Text>
      </View>
    </View>
  );
  ...
  return (
    <SafeAreaView style={listStyles.container}>
      ...
      <TouchableOpacity style={listStyles.fab} onPress={handlePress}>
