import { useNavigate, Form, useNavigation, useLoaderData, useActionData, useParams } from "react-router";
import { useEffect, useState } from "react";

// Helper to build the URL dynamically
const get_server_url = (id: string | undefined) => `https://msaidizi.nsaro.com/api/productdetails/${id}/`;

// 1. Loader: extracts 'id' from params
export async function clientLoader({ params }: any) {
    const response = await fetch(get_server_url(params.productId), {
        headers: {
...
export async function clientAction({ request, params }: any) {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            return { 
                status: "error", 
                message: errorData?.detail || errorData?.message || "Failed to update product." 
            };
        }
...
export default function Product() {
    ...
    const inputStyle = "w-full border-gray-300 border rounded-lg p-2.5 text-gray-500 focus:text-gray-900 focus:ring-2 focus:ring-blue-500 outline-none transition bg-gray-50 focus:bg-white";
    ...
    return (
        <div className="min-h-screen bg-gray-100 p-6 flex items-center justify-center relative">
            ...
            {showToast && actionData && (
                <div className={`fixed top-5 right-5 z-50 flex items-center p-4 w-full max-w-xs text-white rounded-lg shadow-2xl animate-bounce-short ${
                    actionData.status === "success" ? "bg-emerald-600" : "bg-red-600"
                }`}>
                    <div className="text-sm font-medium flex-1">
                        {actionData.message}
                    </div>
                    <button onClick={() => setShowToast(false)} className="ml-4 text-white hover:text-gray-200">✕</button>
                </div>
            )}
            ...
            <div className="max-w-2xl w-full bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-200">
                <div className="bg-white border-b p-6 flex justify-between items-center">
                    <div>
                        <h1 className="text-xl font-bold text-gray-800">Product Settings</h1>
                        <p className="text-gray-400 text-xs uppercase tracking-wider font-semibold">Inventory ID: {product.id}</p>
                    </div>
                    <button onClick={() => navigate(-1)} className="text-gray-400 hover:text-gray-600 text-2xl leading-none">&times;</button>
                </div>
                ...
                <Form method="post" className="p-8 grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" name="price" className={inputStyle} defaultValue={product.price} />
                    </div>

                    <div>
                        <label className="text-xs font-bold text-gray-500 uppercase ml-1">Buying Price</label>
                        <input type="number" step="0.01" name="buying_price" className={inputStyle} defaultValue={product.buying_price} />
                    </div>
