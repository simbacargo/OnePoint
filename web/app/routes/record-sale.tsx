...
const CREATE_SALE_URL = "https://msaidizi.nsaro.com/create/";
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
            <label className="block text-[10px] font-bold text-gray-400 uppercase mb-2">Customer Details (Optional)</label>
            <input 
              type="text" 
              value={customerName}
                ...
                {filteredResults.map((p) => (
                  ...
                  >
                    <div>
                      ...
                      <div className="text-xs text-gray-500 font-mono bg-gray-100 px-1 inline-block rounded">{p.part_number}</div>
                    </div>
                    <div className="text-right">
                      <div className="text-sm font-bold text-gray-900">{Number(p.price).toLocaleString()} TZS</div>
                      <div className={`text-[10px] font-bold ${p.quantity > 0 ? 'text-green-600' : 'text-red-500'}`}>
                        Stock: {p.quantity}
                      </div>
                    </div>
                  </button>
                  ...
                  <tr key={item.id} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center justify-center gap-3">
                        <button onClick={() => updateQty(item.id, -1)} className="w-7 h-7 rounded-full border border-gray-200 hover:bg-gray-100"><i className="bi bi-dash"></i></button>
                        <span className="font-bold text-sm">{item.qty}</span>
                        <button onClick={() => updateQty(item.id, 1)} className="w-7 h-7 rounded-full border border-gray-200 hover:bg-gray-100"><i className="bi bi-plus"></i></button>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-right text-xs text-gray-500">{Number(item.price).toLocaleString()}</td>
                    <td className="px-6 py-4 text-right font-bold text-gray-900 pr-10">{(item.price * item.qty).toLocaleString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="bg-white border border-gray-200 rounded-2xl shadow-sm p-6 sticky top-6">
            ...
            <div className="space-y-4 mb-8">
              <div className="flex justify-between items-baseline border-b border-dashed pb-4">
                ...
                <div className="text-right">
                  <span className="text-3xl font-black text-blue-600 leading-none">{total.toLocaleString()}</span>
