...
const STATS = [
  { label: "Total Revenue", value: "4,250,000 TZS", change: "+12.5%", icon: "bi-currency-exchange", color: "text-blue-600", bg: "bg-blue-50" },
  { label: "Orders Today", value: "48", change: "+5.2%", icon: "bi-cart-check", color: "text-emerald-600", bg: "bg-emerald-50" },
  { label: "Low Stock Items", value: "12", change: "Critical", icon: "bi-exclamation-triangle", color: "text-amber-600", bg: "bg-amber-50" },
  { label: "New Customers", value: "156", change: "+18%", icon: "bi-people", color: "text-purple-600", bg: "bg-purple-50" },
];
...
const RECENT_SALES = [
  ...
  { id: "#1022", product: "Brake Pads - Hilux", amount: "45,000", status: "Verified" },
        ...
        {STATS.map((stat, i) => (
          <div key={i} className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
            <div className="flex justify-between items-start mb-4">
              ...
              <span className={`text-xs font-bold px-2 py-1 rounded-lg ${stat.change.includes('+') ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                {stat.change}
              </span>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <tbody className="divide-y divide-gray-50">
              {RECENT_SALES.map((sale) => (
                <tr key={sale.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-sm font-bold text-gray-800">{sale.product}</td>
                  <td className="px-6 py-4 text-sm font-medium">{sale.amount} TZS</td>
        ...
        <div className="space-y-6">
          <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-sm font-bold text-gray-900">Total Rubia 4L</p>
                  <p className="text-xs text-gray-500">Only 2 units remaining</p>
                </div>
              </div>
              <div className="flex items-center gap-4">
                <div className="w-2 h-10 bg-amber-400 rounded-full"></div>
                <div>
                  <p className="text-sm font-bold text-gray-900">Shell Helix 1L</p>
                  <p className="text-xs text-gray-500">Only 8 units remaining</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-blue-600 p-6 rounded-2xl shadow-lg shadow-blue-200 text-white">
            ...
            <p className="text-blue-100 text-sm mb-4">Check out our latest training manual for inventory management.</p>
