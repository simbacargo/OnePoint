  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const productData = {
      ...
      vehicles: vehiclesString 
        ? vehiclesString.split(',').map(id => parseInt(id.trim(), 10)).filter(id => !isNaN(id)) 
        : []
    };
          ...
          <form onSubmit={handleSubmit} className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="md:col-span-2">
                ...
                <input name="name" type="text" placeholder="e.g. TOTAL RUBIA" className={inputClass} required />
              </div>
              
              <div>
                <label className={labelClass}>Brand / Size</label>
                <input name="brand" type="text" placeholder="e.g. 1L or 4L" className={inputClass} />
              </div>

              <div>
                <label className={labelClass}>Part Number</label>
                <input name="part_number" type="text" placeholder="e.g. SAE 40" className={inputClass} />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                ...
                <input name="price" type="number" step="0.01" placeholder="0.00" className={inputClass} required />
              </div>
              
              <div>
                <label className={labelClass}>Initial Quantity</label>
                <input name="quantity" type="number" placeholder="0" className={inputClass} required />
              </div>

              <div>
                <label className={labelClass}>Vehicle IDs (Comma separated)</label>
                <input name="vehicles" type="text" placeholder="1, 2, 3" className={inputClass} />
              </div>
            </div>

            <div>
              <label className={labelClass}>Description (Optional)</label>
              <textarea name="description" rows={3} placeholder="Add any extra details..." className={inputClass}></textarea>
            </div>

            <div className="flex items-center justify-end gap-3 pt-4">
              <button type="button" onClick={() => navigate("/products")} className="px-6 py-2 text-gray-600 hover:bg-gray-100 rounded-lg transition-colors">
                Cancel
              </button>
              <button 
                type="submit" 
        ...
        <div className="space-y-6">
          <div className="bg-blue-50 border border-blue-100 rounded-xl p-5">
            ...
            <ul className="text-sm text-blue-700 space-y-3">
              <li>• Enter <strong>Vehicle IDs</strong> separated by commas to link this product to specific vehicles.</li>
              <li>• Ensure <strong>Price</strong> is the amount you want to sell at.</li>
            </ul>
          </div>
        </div>
      </div>
